...
ansatzForestAIEpsilon = actual == expected
  where
    (_,eps6,_) = mkAnsatzTensorFastAbs 6 symList6 areaList6 :: (AnsatzForestEta, AnsatzForestEpsilon, ATens 0 1 0 1 0 0 AnsVarR)
...
ansatz14_1Fast = rank == 110
  where
    ...
    (_,_,ans) = mkAnsatzTensorFast 14 symList14_1 evalL :: (AnsatzForestEta, AnsatzForestEpsilon, STTens 0 14 AnsVarR)
...
ansatz14_1Incr = rank == 110
  where
    evalL = map (\(x,_,_) -> x) areaList14_1
    (_,_,ans) = mkAnsatzTensorIncremental 14 symList14_1 evalL :: (AnsatzForestEta, AnsatzForestEpsilon, STTens 0 14 AnsVarR)
    rank = tensorRank2' ans
...
ansatz14_2Fast = rank == 72
  where
    (_,_,ans) = mkAnsatzTensorFastAbs 14 symList14_2 areaList14_2 :: (AnsatzForestEta, AnsatzForestEpsilon, ATens 0 3 0 1 0 0 AnsVarR)
...
ansatz14_2Incr = rank == 72
  where
    (_,_,ans) = mkAnsatzTensorIncrementalAbs 14 symList14_2 areaList14_2 :: (AnsatzForestEta, AnsatzForestEpsilon, ATens 0 3 0 1 0 0 AnsVarR)
    rank = tensorRank6' ans
